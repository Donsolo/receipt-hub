    ...
    return (
        <div className="min-h-screen bg-[var(--bg)] text-[var(--text)] p-8">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-6">
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-4">
                        <div className="flex-1 min-w-0">
                            <h2 className="text-xl font-semibold text-[var(--text)]">My Uploads</h2>
                        </div>
                        <div className="flex flex-wrap items-center gap-3">
                            <select
                                value={selectedUploadCategory}
                                onChange={(e) => setSelectedUploadCategory(e.target.value)}
                                className="bg-[var(--card)] border border-[var(--border)] text-sm rounded-lg px-3 h-11 text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-indigo-500 w-full sm:w-auto"
                            >
                ...
                {/* Content Card */}
                <div className="bg-[var(--card)] rounded-xl shadow border border-[var(--border)] p-6">
                    {loading ? (
                        <p className="text-[var(--muted)]">Loading...</p>
                    ) : receipts.length === 0 ? (
                        <div className="text-center py-12 text-[var(--muted)] border-2 border-dashed border-[var(--border)] rounded-lg">
                            <p>No uploaded receipts found.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {receipts.map((receipt) => (
                                <div key={receipt.id} className="relative group bg-[var(--card)] rounded-lg overflow-hidden border border-[var(--border)] shadow-sm flex flex-col">
                                    <div
                                        ...
                                        {(receipt.clientName || receipt.total) && (
                                            <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/80 to-transparent p-3 pointer-events-none">
                                                <div className="flex justify-between items-start">
                                                    <span className="text-sm font-medium text-[var(--text)] truncate drop-shadow-md">
                                    ...
                                    <div className="bg-[var(--card)] p-3 border-t border-[var(--border)] flex flex-col z-10">
                                        <div className="flex justify-between items-center mb-2">
                                            <span className="text-xs text-[var(--muted)]">
                                                {new Date(receipt.date || receipt.createdAt).toLocaleDateString()}
                                            </span>
                                            {receipt.isFinalized && (
                                                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-100/10 text-[var(--muted)] border border-gray-100/20 uppercase tracking-wider">
                                                    Finalized
                                                </span>
                                            )}
                                        </div>
                                        {!receipt.isFinalized && (
                                            <div className="flex justify-between w-full border-t border-[var(--border)] pt-2 mt-1 gap-2">
                                                <button
                                                ...
                                                <button
                                                    ...
                                                    className="text-[var(--muted)] hover:text-[var(--text)] text-xs font-medium tracking-wide transition-colors"
                                                >
                                                    Edit
                                                </button>
                                                <button
            ...
            {editingReceipt && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--bg)] bg-opacity-75 p-4 sm:p-8">
                    <div className="bg-[var(--card)] rounded-xl shadow-2xl border border-[var(--border)] w-full max-w-md overflow-hidden">
                        <div className="px-6 py-4 border-b border-[var(--border)] flex justify-between items-center">
                            <h3 className="text-lg font-medium text-[var(--text)]">Edit Receipt Details</h3>
                            <button onClick={() => setEditingReceipt(null)} className="text-[var(--muted)] hover:text-[var(--text)]">
                        ...
                        <form onSubmit={handleSaveEdit} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-[var(--muted)] mb-1">Store / Client Name</label>
                                <input
                                    type="text"
                                    value={editForm.clientName}
                                    onChange={(e) => setEditForm({ ...editForm, clientName: e.target.value })}
                                    className="w-full bg-[var(--card)] border border-[var(--border)] rounded-lg px-3 py-2 text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                    placeholder="e.g. Home Depot"
                                />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-[var(--muted)] mb-1">Total ($)</label>
                                    <input
                                        type="number"
                                        step="0.01"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-[var(--muted)] mb-1">Tax ($)</label>
                                    <input
                                        type="number"
                                        step="0.01"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-[var(--muted)] mb-1">Notes</label>
                                <textarea
                                    value={editForm.notes}
                                    onChange={(e) => setEditForm({ ...editForm, notes: e.target.value })}
                                    rows={3}
                                    className="w-full bg-[var(--card)] border border-[var(--border)] rounded-lg px-3 py-2 text-[var(--text)] focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none"
                            ...
                            <div className="pt-4 flex justify-end gap-3">
                                <button
                                    ...
                                    className="px-4 py-2 text-sm font-medium text-[var(--text)] hover:text-[var(--text)] transition-colors"
                                >
                                    Cancel
                                </button>
                                <button
                                    type="submit"
                                    disabled={saving}
                                    className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-[var(--text)] text-sm font-medium rounded-lg disabled:opacity-50 transition-colors shadow-sm"
                                >
            ...
            {selectedImage && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--bg)] bg-opacity-90 p-4 sm:p-8">
                    <div className="relative w-full h-full flex flex-col items-center justify-center max-w-5xl mx-auto">
                        <button
                            onClick={() => setSelectedImage(null)}
                            className="absolute top-4 right-4 text-[var(--text)] hover:text-[var(--text)] bg-[var(--bg)]/50 rounded-full p-2 z-50 transition-colors"
                        >
                        ...
                        <img
                            ...
                            className="max-h-full max-w-full object-contain rounded border border-[var(--border)] shadow-2xl"
                        />
                    </div>
                </div>
            )}
            ...
            {isBundleModalOpen && selectedReceiptForBundle && (
                <div className="fixed inset-0 z-[60] bg-[var(--bg)]/80 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-[var(--bg)] border border-[var(--border)] rounded-xl w-full max-w-md shadow-2xl flex flex-col max-h-[85vh]">
                        <div className="p-6 border-b border-[var(--border)] flex justify-between items-center shrink-0">
                            <h2 className="text-lg font-semibold text-[var(--text)]">Add to Bundle</h2>
                            <button onClick={() => setIsBundleModalOpen(false)} className="text-[var(--muted)] hover:text-[var(--text)] transition-colors">
                        ...
                        <div className="p-6 overflow-y-auto flex-1">
                            <p className="text-sm text-[var(--muted)] mb-6 font-medium">
                                Select a bundle to add <strong className="text-[var(--text)]">{selectedReceiptForBundle.clientName || 'this receipt'}</strong> to.
                            </p>

                            {isBundlesLoading ? (
                                <div className="text-center py-8 text-[var(--muted)] animate-pulse">Loading bundles...</div>
                            ) : userBundles.length === 0 ? (
                                <div className="text-center py-8">
                                    <p className="text-[var(--muted)] text-sm mb-4">You have no bundles.</p>
                                ...
                                <div className="space-y-3">
                                    {userBundles.map(bundle => (
                                        <div key={bundle.id} className="flex items-center justify-between p-3 rounded-lg border border-[var(--border)] hover:border-gray-600 hover:bg-[var(--card-hover)] transition-colors">
                                            <div className="flex-1 pr-4">
                                                <p className="text-sm font-medium text-[var(--text)] line-clamp-1">{bundle.name}</p>
                                                <p className="text-xs text-[var(--muted)] mt-1">{bundle.receiptCount} receipts</p>
                                            </div>
                                            <button
                        ...
                        <div className="p-4 border-t border-[var(--border)] bg-[var(--bg)] rounded-b-xl text-center shrink-0">
                            <Link href="/history?view=bundles" className="text-xs font-medium text-[var(--muted)] hover:text-indigo-400 transition-colors">
                                Manage Bundles
                            </Link>
                        </div>
                    </div>
