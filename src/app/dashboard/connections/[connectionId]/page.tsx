...
export default async function ConnectionProfilePage({ params }: { params: { connectionId: string } }) {
    ...
    const otherUser = connection.requesterId === authUserId ? connection.receiver : connection.requester;
    const primary = otherUser.businessName?.trim() || otherUser.name?.trim() || otherUser.email?.trim() || "Unknown User";
    const secondary = otherUser.businessName ? (otherUser.name || 'Personal Account') : 'Personal Account';
    const email = otherUser.email;
    const logoPath = otherUser.businessLogoPath;
