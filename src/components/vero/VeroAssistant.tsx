    ...
    return (
        <div className={clsx(
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                {messages.filter((m: any) => (m.content && m.content.trim() !== '') || (m.toolInvocations && m.toolInvocations.length > 0)).map((m: any) => (
                    <div key={m.id} className={clsx("flex w-full", m.role === 'user' ? "justify-end" : "justify-start")}>
                        <div className={clsx(
                            ...
                            {m.toolInvocations && m.toolInvocations.length > 0 && (
                                <div className={clsx("flex flex-col gap-2", m.content && m.content.trim() !== '' ? "mt-3" : "")}>
                                    {m.toolInvocations.map((tool: any, idx: number) => (
                                        <div key={idx} className="flex items-center gap-2 text-xs font-medium text-indigo-500 bg-indigo-50 dark:bg-indigo-500/10 px-3 py-2 rounded-lg border border-indigo-100 dark:border-indigo-500/20">
                                            {tool.state === 'call' ? (
                                                <>
