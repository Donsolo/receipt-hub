    ...
    return (
        <>
            <div className="flex items-center justify-center gap-2">
                ...
                {/* View & Print */}
                {invoice.publicToken ? (
                    <Link href={`/invoice/${invoice.publicToken}`} target="_blank" className="text-[var(--muted)] hover:text-blue-500 transition-colors p-1 hidden sm:block" title="View & Print Invoice">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    </Link>
                ) : (
                    <button onClick={handleCopyLink} disabled={isLoading} className={clsx("transition-colors p-1 hidden sm:block", isLoading ? "cursor-not-allowed opacity-50 text-[var(--muted)]" : "text-[var(--muted)] hover:text-blue-500")} title="Generate Link to View">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                    </button>
                )}
